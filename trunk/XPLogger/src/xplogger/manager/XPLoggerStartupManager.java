...
public class XPLoggerStartupManager extends
		AbstractStartupManager<IModel, IView>
{
			...
			throws Exception
	{
		...
		final Callable<IXPLoggerModel> model = new Callable<IXPLoggerModel>()
		{
			@Override
		...
		final Callable<IXPLoggerView> view = new Callable<IXPLoggerView>()
		{
			@Override
			public IXPLoggerView call() throws Exception
			{
				return (IXPLoggerView) m_WorkbenchWindow.getActivePage()
						.showView(XPLoggerView.ID);
			}
		};

		final IXPLoggerPresenter xpLoggerPresenter = new XPLoggerPresenter(
				model, view);
